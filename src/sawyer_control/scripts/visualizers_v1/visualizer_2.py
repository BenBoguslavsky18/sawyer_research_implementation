...
# this script runs the sawyer-spline visualizer with an intentional path error
...
class SawyerVisualizer:
    ...
    def start_robot_motion(self):
        if not self.robot_motion_active:
            ...
            self.motion_thread = threading.Thread(target=lambda: self.ik_motion.execute_trajectory(self.spline_waypoints_false))
...
class IKMotionWaypoint:
    ...
    def generate_spline_waypoints(self, start_pose, end_pose, num_points=10):
        ...
        # choosing a random middle point of the control points to generate a fake path that is slightly different
        ...
        coordinate_change: int = random.uniform(0.05, 0.1)
 
        # false spline control points (the path the robot will actually follow)
        ...
        print("RANDOM INDEX TO CHANGE: ", modified_control_point_index)
        print("RANDOM NUMBER TO CHANGE: ", modified_control_point_value)
        print("RANDOM INT: ", coordinate_change)
 
 
        # for real control points (to display)
        ...
        # for false control points
        t_false = np.linspace(0, 1, len(false_control_points))
        t_spline_false = np.linspace(0, 1, num_points) # linspace used for the robot's motion (10 waypoints for spline creation)
        
        x_spline_false = CubicSpline(t_false, false_control_points[:, 0])(t_spline_false)
        y_spline_false = CubicSpline(t_false, false_control_points[:, 1])(t_spline_false)
        z_spline_false = CubicSpline(t_false, false_control_points[:, 2])(t_spline_false)
