...
class IKMotionWaypoint:
    ...
    def generate_spline_waypoints(self, start_pose, end_pose, num_points=20):
        """
        Creates an even more complex maze-like path with sharper turns and more obstacles.
        This simulates a more challenging maze navigation scenario.
        """
        ...
        control_points = np.array([
            ...
            # Final approach with elevation change
            [0.7, 0.4, 0.35],
            [0.72, 0.42, 0.32],
            [0.74, 0.46, 0.31],
            [0.75, 0.48, 0.3],
        ...
        for i in range(len(y_maze_path) - 1):
            segment_points = 30  # Consistent segments for complex path
            
            y_interpolated = np.linspace(y_maze_path[i], y_maze_path[i+1], segment_points, False)
            z_interpolated = np.linspace(z_maze_path[i], z_maze_path[i+1], segment_points, False)
