...
class SawyerVisualizer:
    def __init__(self):
        ...
        self.background_color = (2, 114, 212) # blue
...
class IKMotionWaypoint:
    ...
    def generate_spline_waypoints(self, start_pose, end_pose, num_points=40): 
        ...
        z_spline_false[26] = z_spline_false[26] + 0.25 # adding error based on interpolated path (more error can be added manually)
    ...
    def execute_trajectory(self, spline_waypoints):
        ...
        """
        error_waypoint_index = 26  # The index where the error occurs
        y_error_position = spline_waypoints[error_waypoint_index].position.y  # Corresponding y-value for the error point
        rospy.loginfo("Error waypoint y-position: {:.2f}".format(y_error_position))

        for i, pose in enumerate(spline_waypoints):
            self.add_waypoint(pose)

            # Publish the error waypoint's y-position
            if i == error_waypoint_index:
                error_waypoint_msg = Float64MultiArray()
                error_waypoint_msg.data = [pose.position.x, y_error_position, pose.position.z]
                rospy.loginfo("Publishing error waypoint (y-position): {:.2f}".format(y_error_position))
