...
#  Sawyer path 1 (Spline) visualizer with modified control point path error
...
start_pose = Pose(position=Point(x=0.6, y=-0.3, z=0.3),
                  orientation=Quaternion(x=0.0, y=1.0, z=0.0, w=0.0))
end_pose = Pose(position=Point(x=0.6, y=0.5, z=0.3),
                orientation=Quaternion(x=0.0, y=1.0, z=0.0, w=0.0))
    ...
    def start_robot_motion(self):
        if self.robot_ready_to_move and not self.robot_motion_active:
            ...
            self.motion_thread = threading.Thread(
                target=self.ik_motion.execute_trajectory,
                args=(self.spline_waypoints_false, self.spline_waypoints_false)  # Pass both waypoints
            )
...
class IKMotionWaypoint:
    ...
    def generate_spline_waypoints(self, start_pose, end_pose, num_points=40): 
        ...
        false_control_points[3, 2] = 0.2  # Setting manual error in control points

        # Publish the error waypoint
        error_waypoint = false_control_points[3]
        self.publish_error_waypoint(error_waypoint)
    ...
    def publish_error_waypoint(self, error_waypoint):
        """
        Publish the error waypoint to the /error_waypoint topic.
        """
        pub = rospy.Publisher('/error_waypoint', Float64MultiArray, queue_size=10, latch=True)
        waypoint_msg = Float64MultiArray()
        waypoint_msg.data = error_waypoint.tolist()
        rospy.sleep(1)  # Ensure the topic is ready before publishing
        pub.publish(waypoint_msg)
        rospy.loginfo(f"Published error waypoint: {error_waypoint}")
    ...
    def execute_trajectory(self, spline_waypoints, spline_waypoints_false):
        """
        Executes a trajectory based on the given spline waypoints while monitoring for error waypoints.
        """
        error_waypoint_pub = rospy.Publisher('/error_waypoint', Float64MultiArray, queue_size=10, latch=True)
        rate = rospy.Rate(10)  # Match controller update rate

        rospy.loginfo("Executing trajectory...")
        for pose in spline_waypoints:
            # Add each waypoint to the trajectory
            success = self.add_waypoint(pose)
            if not success:
                rospy.logerr("Failed to add waypoint: %s", pose)
                continue

            # Check if this waypoint is near the error waypoint
            current_position = [pose.position.x, pose.position.y, pose.position.z]
            error_waypoint = spline_waypoints_false[3].position  # Example: error at 4th waypoint
            distance = np.linalg.norm(np.array([
                error_waypoint.x - current_position[0],
                error_waypoint.y - current_position[1],
                error_waypoint.z - current_position[2]
            ]))

            if distance < 0.03:  # Adjust threshold
                rospy.loginfo("Near error waypoint. Publishing error waypoint and increasing stiffness.")
                error_waypoint_msg = Float64MultiArray()
                error_waypoint_msg.data = [error_waypoint.x, error_waypoint.y, error_waypoint.z]
                error_waypoint_pub.publish(error_waypoint_msg)
                rospy.sleep(1)  # Allow time for the controller to react
