...
#  Sawyer path 1 (Spline) visualizer with modified interpolation points path error
...
end_pose = Pose(position=Point(x=0.6, y=0.5, z=0.3),
                orientation=Quaternion(x=0.0, y=1.0, z=0.0, w=0.0))
def is_near_target(robot_position, target_position, tolerance=0.05):
    """
    Checks if the robot's position is near the target position within a given tolerance.
    Args:
        robot_position: Current position of the robot (geometry_msgs/Point).
        target_position: Target waypoint position (geometry_msgs/Point).
        tolerance: Allowed distance between the current and target positions.
    Returns:
        bool: True if the positions are within the tolerance, otherwise False.
    """
    distance = math.sqrt(
        (robot_position.x - target_position.x) ** 2 +
        (robot_position.y - target_position.y) ** 2 +
        (robot_position.z - target_position.z) ** 2
    )
    rospy.loginfo(f"Checking distance: {distance}, Tolerance: {tolerance}")
...
class IKMotionWaypoint:
    def __init__(self, limb="right"):
        ...
        rospy.Subscriber('/robot/limb/right/endpoint_state', EndpointState, self.update_robot_position)

        rospy.loginfo("IK and MotionWaypoint setup complete.")
        # Initialize the publisher to communicate with the impedance controller
        self.error_trigger_pub = rospy.Publisher('/error_trigger', Bool, queue_size=10)
        self.triggered_error = False

    def update_robot_position(self, msg):
        """
        Updates the current robot position based on feedback.
        """
    ...
    def generate_spline_waypoints(self, start_pose, end_pose, num_points=40): 
        ...
        z_spline_false[25] = z_spline_false[25] + 0.2 # adding error based on interpolated path (more error can be added manually)
    ...
    def execute_trajectory(self, spline_waypoints):
        """
        Executes a trajectory and publishes an error trigger if the robot reaches the critical point.
        """
        rate = rospy.Rate(50)  # 50 Hz for fine-grained control
        for i, pose in enumerate(spline_waypoints):
            rospy.loginfo(f"Waypoint {i}: Target Position: {pose.position}, Current Position: {self.robot_position}")
            
            if i == 23 and not self.triggered_error:
                if is_near_target(self.robot_position, pose.position, tolerance=0.05):
                    rospy.loginfo(f"Error Trigger Condition Met at Waypoint {i}. Publishing error trigger.")
                    self.error_trigger_pub.publish(True)
                    self.triggered_error = True
