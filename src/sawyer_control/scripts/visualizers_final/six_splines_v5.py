...
#  Modified to use limb.move_to_joint_positions() for direct control
...
class SawyerVisualizer:
    ...
    def move_to_start_position(self):
        ...
        def move_and_signal():
            ...
            # Move to start position using direct joint control
            success = self.ik_motion.move_to_pose_direct(start_pose)
        ...
        if self.game_state == "ready_to_play":
            ...
            if self.current_run == 1:
                ...
                def start_trajectory():
                    self.ik_motion.execute_low_impedance_trajectory_direct(self.spline_waypoints)
            else:
                ...
                def start_trajectory():
                    self.ik_motion.execute_tracking_trajectory_direct(self.spline_waypoints)
    ...
    def end_game(self):
        ...
        if self.game_state == "playing":
            ...
            # ENHANCED SAFETY: Stop all robot motion immediately
            rospy.loginfo("EMERGENCY STOP: Halting all robot motion immediately...")
            
            # Stop motion using direct control
            self.ik_motion.emergency_stop()
...
'''ROBOT MOTION WITH DIRECT JOINT CONTROL'''
class IKMotionWaypoint:
    def __init__(self, limb="right"):
        ...
        rospy.loginfo("IK and Direct Joint Motion setup complete.")
    ...
    def calculate_ik(self, pose):
        ...
        return {joint_name: self.ik_joint_positions[i] for i, joint_name in 
                enumerate(['right_j0', 'right_j1', 'right_j2', 'right_j3', 'right_j4', 'right_j5', 'right_j6'])}

    def move_to_pose_direct(self, pose):
        """Move robot to pose using direct joint control with limb.move_to_joint_positions()"""
        joint_angles = self.calculate_ik(pose)
        if joint_angles is None:
            rospy.logerr("Failed to move to pose: IK solution not found")
            return False
        
        rospy.loginfo(f"Moving to pose with joint angles: {joint_angles}")
        
        try:
            # Use limb.move_to_joint_positions() with timeout and speed control
            result = self._limb.move_to_joint_positions(
                joint_angles,
                timeout=15.0,  # 15 second timeout
                threshold=0.008726646  # ~0.5 degrees in radians
            )
            
            if result:
                rospy.loginfo("Successfully moved to pose using direct joint control!")
                return True
            else:
                rospy.logerr("Failed to move to pose: Timeout or motion failed")
                return False
                
        except Exception as e:
            rospy.logerr(f"Error during direct joint motion: {e}")
            return False
    ...
    def execute_low_impedance_trajectory_direct(self, spline_waypoints):
        """
        NORMAL MODE (Run 1): Move directly to final waypoint using direct joint control with low impedance.
        """
        try:
            ...
            rospy.loginfo("You can manually guide the robot arm during motion!")
            
            # Set motion flag
            self.motion_active = True
            self.stop_motion = False
            
            # Use direct joint control with slower speed settings
            self._limb.set_joint_position_speed(0.1)  # Set to 10% of max speed
            
            try:
                # Move to final position with timeout
                success = self._limb.move_to_joint_positions(
                    joint_angles,
                    timeout=30.0,  # Longer timeout for slow motion
                    threshold=0.008726646  # ~0.5 degrees
                )
                
                if success:
                    rospy.loginfo("Successfully completed normal mode motion to final position!")
                else:
                    rospy.logwarn("Motion timed out or was interrupted")
                    
            except Exception as e:
                rospy.logerr(f"Error during normal mode motion: {e}")
            
            # Keep motion flag active but stop actual movement
            self.motion_active = False
    ...
    def execute_tracking_trajectory_direct(self, spline_waypoints):
        """
        TRACKING MODE (Run 2): Execute trajectory through ALL spline waypoints using direct joint control.
        """
        try:
            # Enable low impedance mode first
            self.enable_low_impedance()
            ...
            rospy.loginfo(f"TRACKING MODE: Moving through {len(spline_waypoints)} waypoints...")
            rospy.loginfo("Robot will pull you through ALL spline points!")
            ...
            # Set slower joint speed for tracking mode
            self._limb.set_joint_position_speed(0.15)  # 15% of max speed for tracking
            
            # Move through each waypoint sequentially
            for i, pose in enumerate(spline_waypoints):
                if self.stop_motion or rospy.is_shutdown():
                    rospy.loginfo("Motion stopped by user or shutdown")
                    break
                ...
                rospy.loginfo(f"Moving to waypoint {i+1}/{len(spline_waypoints)}")
                
                try:
                    # Move to this waypoint
                    success = self._limb.move_to_joint_positions(
                        joint_angles,
                        timeout=10.0,  # Shorter timeout per waypoint
                        threshold=0.017453293  # ~1 degree (less precise for smoother tracking)
                    )
                    
                    if not success:
                        rospy.logwarn(f"Waypoint {i+1} motion timed out, continuing...")
                        
                    # Small delay between waypoints for smoother motion
                    rospy.sleep(0.1)
                        
                except Exception as e:
                    rospy.logerr(f"Error moving to waypoint {i+1}: {e}")
                    continue
    ...
    def emergency_stop(self):
        ...
        try:
            # Stop any ongoing motion by commanding current position
            current_joints = self._limb.joint_angles()
            self._limb.move_to_joint_positions(current_joints, timeout=1.0)
        except Exception as e:
            rospy.logwarn(f"Could not stop robot motion: {e}")
