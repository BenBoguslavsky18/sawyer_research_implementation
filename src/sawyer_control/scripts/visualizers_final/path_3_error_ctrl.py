...
class IKMotionWaypoint:
    ...
    def generate_spline_waypoints(self, start_pose, end_pose):
        ...
        false_control_points[3, 2] = 0.25 # setting manual error in control points

        # intended (true) path motion
        x_straight_path = control_points[:, 0]
        y_straight_path = control_points[:, 1]
        z_straight_path = control_points[:, 2]

        # false path motion
        x_straight_path_false = false_control_points[:, 0]
        y_straight_path_false = false_control_points[:, 1]
        z_straight_path_false = false_control_points[:, 2]

        self.waypoints = [Pose(position=Point(x=x, y=y, z=z), orientation=start_pose.orientation)
                          for x, y, z in zip(x_straight_path_false, y_straight_path_false, z_straight_path_false)]
