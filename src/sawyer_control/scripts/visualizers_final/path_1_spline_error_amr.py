...
class IKMotionWaypoint:
    def __init__(self, limb="right"):
        ...
        self.error_waypoint_pub = rospy.Publisher('/error_waypoint', Float64MultiArray, queue_size=10)
        self.trajectory_pub = rospy.Publisher('/trajectory_points', Float64MultiArray, queue_size=10)
        #self.stiffness_update_pub = rospy.Publisher('/stiffness_update', Bool, queue_size=10)
    ...
    def publish_trajectory_points(self, waypoints):
        """
        Publishes the trajectory waypoints to a ROS topic.
        :param waypoints: List of Pose objects representing the trajectory.
        """
        points = Float64MultiArray()
        points.data = []

        for pose in waypoints:
            points.data.extend([pose.position.x, pose.position.y, pose.position.z])

        self.trajectory_pub.publish(points)
        rospy.loginfo("Published trajectory points.")
    ...
    def generate_spline_waypoints(self, start_pose, end_pose, num_points=40): 
        ...
        self.publish_trajectory_points(self.waypoints_false)
    ...
    def execute_trajectory(self, spline_waypoints):
        ...
        """
        error_waypoint_index = 25  # The index where the error occurs
        for i, pose in enumerate(spline_waypoints):
            self.add_waypoint(pose)

            # Publish the error waypoint's position
            if i == error_waypoint_index:
                error_waypoint_msg = Float64MultiArray()
                error_waypoint_msg.data = [pose.position.x, pose.position.y, pose.position.z]
                rospy.loginfo("Publishing error waypoint: %s", error_waypoint_msg.data)
                self.error_waypoint_pub.publish(error_waypoint_msg)
