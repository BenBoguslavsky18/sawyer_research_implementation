...
class SawyerVisualizer:
    def __init__(self):
        ...
        self.current_level = 0  # Start with level 0 (first challenge)
        self.total_levels = NUM_LEVELS
        self.level_completion_times = []  # Track completion time for each level
        self.current_challenge = None
        
        # Load first challenge
        self.load_challenge(self.current_level)
        ...
        self.level_complete_delay = 3.0  # Show level complete for 3 seconds before next level

    def load_challenge(self, level_index):
        """Load a specific challenge/spline configuration"""
        if level_index >= len(SPLINE_CHALLENGES):
            rospy.logerr(f"Invalid level index: {level_index}")
            return
            
        self.current_challenge = SPLINE_CHALLENGES[level_index]
        rospy.loginfo(f"Loading {self.current_challenge['name']}")
        
        # CRITICAL: Create a completely fresh motion interface for each level
        rospy.loginfo("Creating fresh robot motion interface...")
        self.ik_motion = IKMotionWaypoint()  # Brand new instance
        ...
        spline_type = self.current_challenge.get('type', 'spline')  # Default to spline if not specified
        ...
        # Recalculate display points and target pixel position
        x_coords = [p[0] for p in self.display_waypoints]
        y_coords = [p[1] for p in self.display_waypoints]
        x_coords = self.convert_to_pixels_x(x_coords, -0.4, 0.6, 200, 1720)  # Expanded range for new challenges
        y_coords = self.convert_to_pixels_y(y_coords, 0.1, 0.5, 200, 1000)
        self.smooth_points = list(zip(x_coords, y_coords))
        
        # Calculate final target position in pixels for current challenge
        final_y = end_pose.position.y
        final_z = end_pose.position.z
        target_x_pixel = self.convert_to_pixels_x([final_y], -0.4, 0.6, 200, 1720)[0]  # Expanded range
        target_y_pixel = self.convert_to_pixels_y([final_z], 0.1, 0.5, 200, 1000)[0]
        self.final_target_pixel = (int(target_x_pixel), int(target_y_pixel))
        
        rospy.loginfo(f"Challenge loaded with fresh motion interface: {self.current_challenge['name']}")

    def advance_to_next_level(self):
        """Advance to the next challenge level"""
        self.level_completion_times.append(self.game_completion_time)
        self.current_level += 1
        
        if self.current_level >= self.total_levels:
            # All levels completed!
            rospy.loginfo("🎉 ALL CHALLENGES COMPLETED! 🎉")
            self.show_final_results()
        else:
            # Load next challenge (this will create a fresh motion interface)
            rospy.loginfo(f"Advancing to level {self.current_level + 1}...")
            self.game_state = "level_transition"
            self.level_transition_timer = time.time()
            
    def show_final_results(self):
        """Display final results for all completed levels"""
        rospy.loginfo("=== FINAL GAME RESULTS ===")
        total_time = sum(self.level_completion_times)
        rospy.loginfo(f"Total completion time: {total_time:.2f} seconds")
        for i, completion_time in enumerate(self.level_completion_times):
            rospy.loginfo(f"{SPLINE_CHALLENGES[i]['name']}: {completion_time:.2f}s")
        rospy.loginfo("Congratulations on completing all challenges!")
        
        # Set up final game over state
    ...
    def move_to_start_position(self):
        ...
        rospy.loginfo(f"Moving to start position for {self.current_challenge['name']}...")
        ...
        def move_and_signal():
            ...
            # Move to start position (the fresh motion interface should handle this properly)
            success = self.ik_motion.move_to_pose(start_pose)
            
            if not rospy.is_shutdown():
                if success is not False:  # move_to_pose doesn't return False on failure, just logs error
                    self.robot_ready_to_move = True
                    self.game_state = "ready_to_play"
                    self.spacebar_disabled = False  # Re-enable spacebar for next action
                    rospy.loginfo(f"Successfully reached start position. Ready for {self.current_challenge['name']}!")
                else:
    ...
    def check_success(self):
        ...
        if self.game_state == "playing" and not self.game_won:
            ...
            if distance <= self.success_radius:
                ...
                rospy.loginfo(f"=== {self.current_challenge['name']} COMPLETED in {self.game_completion_time:.2f} seconds ===")
                self.end_game()
    ...
    def end_game(self):
        ...
        if self.game_state == "playing":
            ...
            # Print final results
            rospy.loginfo("=== LEVEL RESULTS ===")
            if self.game_won:
                rospy.loginfo(f"✅ {self.current_challenge['name']} COMPLETED in {self.game_completion_time:.2f} seconds")
                # Check if this was the last level
                if self.current_level >= self.total_levels - 1:
                    # This was the final level
                    self.advance_to_next_level()  # This will trigger final results
                else:
                    # More levels to go
                    self.advance_to_next_level()
                    return  # Don't start auto-close timer yet
            else:
                rospy.loginfo(f"❌ {self.current_challenge['name']} FAILED! Time ran out.")
            ...
            # Start auto-close timer only for final completion or failure
            if self.game_state == "all_levels_complete" or not self.game_won:
                self.show_results_timer = time.time()
                if self.game_state == "all_levels_complete":
                    rospy.loginfo(f"Game will close automatically in {self.auto_close_delay} seconds...")
                else:
                    rospy.loginfo(f"Press SPACEBAR to retry or close window")
    ...
    def draw_level_info(self):
        ...
        if self.current_challenge:
            level_text = f"Level {self.current_level + 1}/{self.total_levels}: {self.current_challenge['name']}"
            level_surface = self.small_font.render(level_text, True, (255, 255, 255))
            level_rect = level_surface.get_rect()
            level_rect.topleft = (20, 60)  # Below the distance info
    ...
    def draw_game_result(self):
        ...
        if self.game_state == "game_over" or self.game_state == "all_levels_complete" or (self.game_state == "playing" and (self.game_won or self.game_lost)):
            if self.game_won and self.game_state == "all_levels_complete":
                result_text = "ALL LEVELS COMPLETE!"
                text_color = (255, 215, 0)  # Gold color
                total_time = sum(self.level_completion_times)
                completion_text = f"Total Time: {total_time:.1f}s"
            elif self.game_won:
                if self.current_level >= self.total_levels - 1:
                    result_text = "FINAL LEVEL COMPLETE!"
                    text_color = (255, 215, 0)  # Gold for final level
                else:
                    result_text = "LEVEL COMPLETE!"
                    text_color = (0, 255, 0)  # Green for regular levels
    ...
    def run(self):
        ...
        while running and not rospy.is_shutdown():
            # Check if we should auto-close after showing final results
            if (self.game_state == "all_levels_complete" and 
                self.show_results_timer and 
                time.time() - self.show_results_timer >= self.auto_close_delay):
                rospy.loginfo("Auto-closing game window after showing final results...")
                running = False
            
            # Handle level transitions
            if (self.game_state == "level_transition" and 
                hasattr(self, 'level_transition_timer') and
                time.time() - self.level_transition_timer >= self.level_complete_delay):
                self.load_challenge(self.current_level)
                self.game_state = "waiting_to_start"
                self.spacebar_disabled = False
                self.robot_ready_to_move = False
                # Reset ALL game variables for new level
                self.trail_points = []
                self.success_achieved = False
                self.game_won = False
                self.game_lost = False
                ...
                self.final_distance = None
                self.game_completion_time = None
                rospy.loginfo(f"Level {self.current_level + 1} ready - Press SPACEBAR to move to start position")
            ...
            self.draw_level_info()  # Show current level info
