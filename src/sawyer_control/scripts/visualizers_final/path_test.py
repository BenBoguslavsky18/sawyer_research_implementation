...
class IKMotionWaypoint:
    ...
    def generate_spline_waypoints(self, start_pose, end_pose):
        ...
        for i in range(len(y_straight_path) - 1):
            interpolated_values = np.linspace(y_straight_path[i], y_straight_path[i+1], 400, False)
            y_straight_path_display.extend(interpolated_values)
        z_straight_path_display = []
        for i in range(len(z_straight_path) - 1):
            interpolated_values = np.linspace(z_straight_path[i], z_straight_path[i+1], 400, False)
