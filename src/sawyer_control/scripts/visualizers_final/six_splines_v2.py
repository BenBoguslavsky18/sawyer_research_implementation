...
def restart_for_next_level(current_level):
    """Restart the program for the next level"""
    next_level = current_level + 1
    
    if next_level >= NUM_LEVELS:
        rospy.loginfo("🎉 ALL LEVELS COMPLETED! 🎉")
        return False
    
    rospy.loginfo(f"Restarting for level {next_level + 1}...")
    
    # Set environment variable for next level
    env = os.environ.copy()
    env['SAWYER_LEVEL'] = str(next_level)
    
    # Restart the script with the next level
...
class SawyerVisualizer:
    def __init__(self, level_index=0):
        ...
        # Single level variables (no progression)
        self.current_level = level_index
        self.total_levels = NUM_LEVELS
        self.current_challenge = SPLINE_CHALLENGES[level_index]
        
        # Update window caption with current level
        pygame.display.set_caption(f"Sawyer 30-Second Challenge - {self.current_challenge['name']}")
        
        rospy.loginfo(f"Starting {self.current_challenge['name']} (Level {level_index + 1}/{NUM_LEVELS})")
        ...
        x_coords = self.convert_to_pixels_x(x_coords, -0.4, 0.6, 200, 1720)
        ...
        target_x_pixel = self.convert_to_pixels_x([final_y], -0.4, 0.6, 200, 1720)[0]
        ...
        self.restart_next_level = False  # Flag to trigger restart
    ...
    def draw_instructions(self):
        ...
        if self.game_state == "waiting_to_start":
            ...
            if self.game_won:
                if self.current_level >= self.total_levels - 1:
                    instruction_text = "🎉 ALL LEVELS COMPLETED! Window will close automatically..."
                else:
    ...
    def run(self):
        ...
        while running and not rospy.is_shutdown():
            # Check if we should auto-close and restart for next level
            if (self.game_state == "game_over" and 
                self.show_results_timer and 
                time.time() - self.show_results_timer >= self.auto_close_delay):
                
                if self.restart_next_level and self.current_level < self.total_levels - 1:
                    # Restart for next level
                    rospy.loginfo("Closing pygame and restarting for next level...")
                    restart_for_next_level(self.current_level)
                    running = False
                elif self.current_level >= self.total_levels - 1:
                    # Final level completed
                    rospy.loginfo("All levels completed! Closing game...")
                    running = False
                else:
                    # Failed level, just close
                    running = False
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:
                        if self.game_state == "waiting_to_start":
                            ...
                            # Restart the current level by restarting the program
                            rospy.loginfo("Restarting current level...")
                            restart_for_next_level(self.current_level - 1)  # Restart same level
                            running = False
...
class IKMotionWaypoint:
    def __init__(self, limb="right"):
        ...
        self.low_stiffness = [5.0, 5.0, 5.0, 5.0, 5.0, 5.0]  # Increased X stiffness to prevent drift
...
if __name__ == '__main__':
    try:
        # Get the current level from command line or environment
        ...
        rospy.loginfo(f"Starting Sawyer Challenge at Level {current_level + 1}")
        
        # Create visualizer for the specific level
        sawyerVisualizer = SawyerVisualizer(current_level)
